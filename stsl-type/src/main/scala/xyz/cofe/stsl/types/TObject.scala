...
class TObject(Name: String,
   ...
   */
  override def typeVarReplace(recipe: TypeVariable => Option[Type])(implicit trace: TypeVarTracer): TObject = {
    require(recipe != null)

    trace("TObject")(if (generics.isEmpty) {
      trace("generics is empty")(this)
    } else {
      trace("generics non empty") {
        var replaceTypeVarMap: Map[String, Type] = Map()

        val replacement: TypeVariable => Option[Type] = (tv) => {
          val trgt = trace(s"recipe($tv)")(recipe(tv))
          if (trgt.isDefined) {
            trace(s"check assignable")(if (!trace(s"replaceTypeVarMap.contains(${tv.name})")(replaceTypeVarMap.contains(tv.name))) {
              if (!trace(s"generics(${tv.name}).assignable(${trgt.get})")(generics(tv.name).assignable(trgt.get))) {
                throw TypeError(s"can't assign type ${trgt.get} into type variable ${generics(tv.name)}")
              }
              trace(s"replaceTypeVarMap insert ${tv.name} -> ${trgt.get}")(replaceTypeVarMap = replaceTypeVarMap + (tv.name -> trgt.get))
            })
          }
          trgt
        }

        val newGeneric = trace(s"generics")(generics match {
        ...
        val newExtend =
          trace("newExtend")(if (extend.isDefined) {
        ...
        val newTvFields: Seq[Field] =
          trace("newTvFields")(fields
        ...
        val newFields = trace("newFields")(Fields((asIsFields ++ newTvFields).toList))

        val newMethods =
          trace("newMethods")(new Methods(
        ...
        val nnewGeneric =
          trace("final generics")(new GenericParams(
