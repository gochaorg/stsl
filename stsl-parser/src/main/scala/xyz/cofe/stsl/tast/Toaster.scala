...
case class Toaster(
   ...
   *
   * [[LambdaAST]] компилируется так:
   *
   *     - создается копия AST (тело LambdaAST)
   *     - находятся все ссылки на переменные, на аргументы ([[IdentifierAST]])
   *     - найденные переменные заменяются на ссылки в стек [[StackedArgumentAST]]
   *     - создается тип, наследник от [[Fun]]. Данную функцию можно вызывать [[CallableFn]]
   *     - в реализации вызова есть лямбда которая:
   *        - перед вызовом засовывает аргументы в стек [[CallStack]] (push)
   *        - вычисляет значение тела функции `body.supplier.get()`
   *        - после вычисления восстанавливает стек [[CallStack]] (pop)
   *
